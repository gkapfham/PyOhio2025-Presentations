...
Scan the QR code to follow along with the presentation on your own device.
:::
...
- Existing tools often focus on **style** not **semantic structure**
- Regex is **brittle** and AST tools are **hard to prototype**

:::

::: {.fragment .fade style="margin-top: -0.25em; font-size: 0.925em;"}
...
## Building a source code analyzer! What are options and their trade-offs?
...
- [Python `ast` module](https://docs.python.org/3/library/ast.html){.preview-link} - The official documentation for Python's Abstract Syntax Tree module.
- [Pylint](https://pylint.org/){.preview-link} - A popular static code analyzer for Python.
- [Flake8](https://flake8.pycqa.org/en/latest/){.preview-link} - A wrapper around PyFlakes, pycodestyle, and McCabe.
...
- [`pyastgrep`](https://github.com/AstuteSource/pyastgrep){.preview-link} - A tool for searching Python code with XPath expressions.
- [`chasten`](https://github.com/AstuteSource/chasten){.preview-link} - A configurable program analysis and linting tool built on `pyastgrep`.
- [`datasette`](https://datasette.io/){.preview-link} - A tool for exploring and publishing data.
